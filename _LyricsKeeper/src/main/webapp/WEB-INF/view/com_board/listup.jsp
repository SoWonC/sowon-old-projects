...
<body>
	...
	<table>
		<tr>
			<td><input type="text" size="60" value="" method=""> 
	...
	<table width="500" cellpadding="0" cellspacing="0" border="1">
		...
		<c:forEach var="com" items="${com_Page.com}">
			<tr>
				<!-- 사용자의 id를 링크로 연결 -->
				<td><a href="<c:url value='/view/${com.number}'/>">${com.number}</a></td>
		...
		<c:if test="${com_Page.hasUser()}">
			<tr>
				...
				<td colspan="10"><c:if test="${com_Page.currentPage >1}">
						<a href="/com/listup?pageNo=${com_Page.currentPage - 1}">[이전]</a>
						...
						<c:choose>
							...
							<c:otherwise>
								<a href="/com/listup?pageNo=${pNo}">${pNo}</a>
							</c:otherwise>
						</c:choose>
					</c:forEach> <c:if test="${com_Page.currentPage < com_Page.totalPages}">
						<a href="/com/listup?pageNo=${com_Page.currentPage + 1}">[다음]</a>
					</c:if></td>
			</tr>
		</c:if>
	</table>
